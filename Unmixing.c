...
char     ersfile[100], binfile[100], tiffile[100], sysstring[200], txtfile[100], enddir[100], txtfile2[100], txtfile3[100];
float    *buffer;
double   **a;
...
txtfile[i]='\0';
sprintf(txtfile, "%s_unmixing_report.txt", binfile);
i=(strlen(binfile));
txtfile2[i]='\0';
sprintf(txtfile2, "%s_endmembers.txt", binfile);
i=(strlen(binfile));
txtfile3[i]='\0';
sprintf(txtfile3, "%s_Unmixing_endmember_spectral.txt", binfile);
...
/* Noise2 Image Writing */
    ...
    for (j=0; j<rows; j++){
        ...
        if (fseek ( fnoise2, cols*(bands-1)*sizeof(float), SEEK_CUR) != 0){
